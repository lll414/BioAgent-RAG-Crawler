---
source_url: https://r4ds.hadley.nz/communication.html
download_date: 2025-11-20
---

1. [Visualize](./visualize.html)
2. [11  Communication](./communication.html)

# 11  Communication

## 11.1 Introduction

In [Chapter 10](EDA.html), you learned how to use plots as tools for *exploration*. When you make exploratory plots, you know—even before looking—which variables the plot will display. You made each plot for a purpose, could quickly look at it, and then move on to the next plot. In the course of most analyses, you’ll produce tens or hundreds of plots, most of which are immediately thrown away.
...
![Scatterplot with math text on the x and y axis labels. X-axis label  says x_i, y-axis label says sum of x_i  squared, for i from 1 to n.](communication_files/figure-html/unnamed-chunk-3-1.png)
...
2. Recreate the following plot using the fuel economy data. Note that both the colors and shapes of points vary by type of drive train.
   ...
   ![Scatterplot of highway versus city fuel efficiency. Shapes and  colors of points are determined by type of drive train.](communication_files/figure-html/unnamed-chunk-4-1.png)
...
![Scatterplot of highway mileage versus engine size where points are colored  by drive type. Smooth curves for each drive type are overlaid.  Text labels identify the curves as front-wheel, rear-wheel, and 4-wheel.](communication_files/figure-html/unnamed-chunk-6-1.png)
...
![Scatterplot of highway fuel efficiency versus engine size of cars. Points  where highway mileage is above 40 as well as above 20 with engine size  above 5 are red, with a hollow red circle, and labelled with model name  of the car.](communication_files/figure-html/unnamed-chunk-8-1.png)
...
![Scatterplot of highway fuel efficiency versus engine size of cars. A red  arrow pointing down follows the trend of the points and the annotation  placed next to the arrow reads "Larger engine sizes tend to have lower  fuel economy". The arrow and the annotation text is red.](communication_files/figure-html/unnamed-chunk-10-1.png)
...
![Scatterplot of highway fuel efficiency versus engine size of cars, colored by drive. The y-axis has breaks starting at 15 and ending at 40,  increasing by 5.](communication_files/figure-html/unnamed-chunk-12-1.png)
...
![Scatterplot of highway fuel efficiency versus engine size of cars, colored  by drive. The x and y-axes do not have any labels at the axis ticks.  The legend has custom labels: 4-wheel, front, rear.](communication_files/figure-html/unnamed-chunk-13-1.png)
...
![Two side-by-side box plots of price versus cut of diamonds. The outliers  are transparent. On both plots the x-axis labels are formatted as dollars. The x-axis labels on the left plot start at $0 and go to $15,000, increasing  by $5,000. The x-axis labels on the right plot start at $1K and go to  $19K, increasing by $6K. ](communication_files/figure-html/unnamed-chunk-14-1.png)

![Two side-by-side box plots of price versus cut of diamonds. The outliers  are transparent. On both plots the x-axis labels are formatted as dollars. The x-axis labels on the left plot start at $0 and go to $15,000, increasing  by $5,000. The x-axis labels on the right plot start at $1K and go to  $19K, increasing by $6K. ](communication_files/figure-html/unnamed-chunk-14-2.png)
...
![Segmented bar plots of cut, filled with levels of clarity. The y-axis  labels start at 0% and go to 100%, increasing by 25%. The y-axis label  name is "Percentage".](communication_files/figure-html/unnamed-chunk-15-1.png)
...
![Line plot of id number of presidents versus the year they started their  presidency. Start year is marked with a point and a segment that starts  there and ends at the end of the presidency. The x-axis labels are  formatted as two digit years starting with an apostrophe, e.g., '53.](communication_files/figure-html/unnamed-chunk-16-1.png)
...
![Four scatterplots of highway fuel efficiency versus engine size of cars  where points are colored based on class of car. Clockwise, the legend  is placed on the right, left, bottom, and top of the plot.](communication_files/figure-html/unnamed-chunk-17-1.png)

![Four scatterplots of highway fuel efficiency versus engine size of cars  where points are colored based on class of car. Clockwise, the legend  is placed on the right, left, bottom, and top of the plot.](communication_files/figure-html/unnamed-chunk-17-2.png)

![Four scatterplots of highway fuel efficiency versus engine size of cars  where points are colored based on class of car. Clockwise, the legend  is placed on the right, left, bottom, and top of the plot.](communication_files/figure-html/unnamed-chunk-17-3.png)

![Four scatterplots of highway fuel efficiency versus engine size of cars  where points are colored based on class of car. Clockwise, the legend  is placed on the right, left, bottom, and top of the plot.](communication_files/figure-html/unnamed-chunk-17-4.png)
...
![Two plots of price versus carat of diamonds. Data binned and the color of  the rectangles representing each bin based on the number of points that  fall into that bin. In the plot on the right, price and carat values  are logged and the axis labels shows the logged values.](communication_files/figure-html/unnamed-chunk-19-1.png)

![Two plots of price versus carat of diamonds. Data binned and the color of  the rectangles representing each bin based on the number of points that  fall into that bin. In the plot on the right, price and carat values  are logged and the axis labels shows the logged values.](communication_files/figure-html/unnamed-chunk-19-2.png)
...
![Plot of price versus carat of diamonds. Data binned and the color of  the rectangles representing each bin based on the number of points that  fall into that bin. The axis labels are on the original data scale.](communication_files/figure-html/unnamed-chunk-20-1.png)

Another scale that is frequently customized is color. The default categorical scale picks colors that are evenly spaced around the color wheel. Useful alternatives are the ColorBrewer scales which have been hand tuned to work better for people with common types of color blindness. The two plots below look similar, but there is enough difference in the shades of red and green that the dots on the right can be distinguished even by people with red-green color blindness.[1](#fn1)
...
![Two scatterplots of highway mileage versus engine size where points are  colored by drive type. The plot on the left uses the default  ggplot2 color palette and the plot on the right uses a different color  palette.](communication_files/figure-html/unnamed-chunk-21-1.png)

![Two scatterplots of highway mileage versus engine size where points are  colored by drive type. The plot on the left uses the default  ggplot2 color palette and the plot on the right uses a different color  palette.](communication_files/figure-html/unnamed-chunk-21-2.png)
...
![Scatterplot of highway mileage versus engine size where both color  and shape of points are based on drive type. The color palette is not  the default ggplot2 palette.](communication_files/figure-html/unnamed-chunk-22-1.png)

The ColorBrewer scales are documented online at <https://colorbrewer2.org/> and made available in R via the **RColorBrewer** package, by Erich Neuwirth. [Figure 11.1](#fig-brewer) shows the complete list of all palettes. The sequential (top) and diverging (bottom) palettes are particularly useful if your categorical values are ordered, or have a “middle”. This often arises if you’ve used `cut()` to make a continuous variable into a categorical variable.

![All colorBrewer scales. One group goes from light to dark colors.  Another group is a set of non ordinal colors. And the last group has  diverging scales (from dark to light to dark again). Within each set  there are a number of palettes.](communication_files/figure-html/fig-brewer-1.png)
...
![Line plot of id number of presidents versus the year they started their  presidency. Start year is marked with a point and a segment that starts  there and ends at the end of the presidency. Democratic presidents are  represented in blue and Republicans in red.](communication_files/figure-html/unnamed-chunk-23-1.png)
...
![Three hex plots where the color of the hexes show the number of observations  that fall into that hex bin. The first plot uses the default, continuous  ggplot2 scale. The second plot uses the viridis, continuous scale, and the  third plot uses the viridis, binned scale.](communication_files/figure-html/unnamed-chunk-24-1.png)

![Three hex plots where the color of the hexes show the number of observations  that fall into that hex bin. The first plot uses the default, continuous  ggplot2 scale. The second plot uses the viridis, continuous scale, and the  third plot uses the viridis, binned scale.](communication_files/figure-html/unnamed-chunk-24-2.png)

![Three hex plots where the color of the hexes show the number of observations  that fall into that hex bin. The first plot uses the default, continuous  ggplot2 scale. The second plot uses the viridis, continuous scale, and the  third plot uses the viridis, binned scale.](communication_files/figure-html/unnamed-chunk-24-3.png)
...
![On the left, scatterplot of highway mileage vs. displacement  where points are colored by drive type.  The smooth curve overlaid shows a decreasing, and then  increasing trend, like a hockey stick. On the right, same variables  are plotted with displacement ranging only from 5 to 6 and highway  mileage ranging only from 10 to 25. The smooth curve overlaid shows a  trend that's slightly increasing first and then decreasing. ](communication_files/figure-html/unnamed-chunk-25-1.png)

![On the left, scatterplot of highway mileage vs. displacement  where points are colored by drive type.  The smooth curve overlaid shows a decreasing, and then  increasing trend, like a hockey stick. On the right, same variables  are plotted with displacement ranging only from 5 to 6 and highway  mileage ranging only from 10 to 25. The smooth curve overlaid shows a  trend that's slightly increasing first and then decreasing. ](communication_files/figure-html/unnamed-chunk-25-2.png)
...
![On the left, a scatterplot of highway mileage vs. displacement of SUVs. On the right, a scatterplot of the same variables for compact cars. Points are colored by drive type for both plots. Among SUVs more of  the cars are 4-wheel drive and the others are rear-wheel drive, while  among compact cars more of the cars are front-wheel drive and the others  are 4-wheel drive. SUV plot shows a clear negative relationship  between higway mileage and displacement while in the compact cars plot  the relationship is much flatter.](communication_files/figure-html/unnamed-chunk-27-1.png)

![On the left, a scatterplot of highway mileage vs. displacement of SUVs. On the right, a scatterplot of the same variables for compact cars. Points are colored by drive type for both plots. Among SUVs more of  the cars are 4-wheel drive and the others are rear-wheel drive, while  among compact cars more of the cars are front-wheel drive and the others  are 4-wheel drive. SUV plot shows a clear negative relationship  between higway mileage and displacement while in the compact cars plot  the relationship is much flatter.](communication_files/figure-html/unnamed-chunk-27-2.png)
...
![Scatterplot of highway mileage vs. displacement of cars, colored by class  of car. The plot background is white, with gray grid lines.](communication_files/figure-html/unnamed-chunk-31-1.png)

ggplot2 includes the eight themes shown in [Figure 11.2](#fig-themes), with `theme_gray()` as the default.[2](#fn2) Many more are included in add-on packages like **ggthemes** (<https://jrnold.github.io/ggthemes>), by Jeffrey Arnold. You can also create your own themes, if you are trying to match a particular corporate or journal style.
...
![Two plots (a scatterplot of highway mileage versus engine size and a  side-by-side boxplots of highway mileage versus drive train) placed next  to each other.](communication_files/figure-html/unnamed-chunk-33-1.png)
