---
source_url: https://r4ds.hadley.nz/quarto.html
download_date: 2025-11-20
---

1. [Communicate](./communicate.html)
2. [28  Quarto](./quarto.html)
...
[Figure 28.1](#fig-diamond-sizes-notebook) shows a `.qmd` document in RStudio with notebook interface where code and output are interleaved. You can run each code chunk by clicking the Run icon (it looks like a play button at the top of the chunk), or by pressing Cmd/Ctrl + Shift + Enter. RStudio executes the code and displays the results inline with the code.

![RStudio window with a Quarto document titled "diamond-sizes.qmd"  on the left and a blank Viewer window on the right. The Quarto  document has a code chunk that creates a frequency plot of diamonds  that weigh less than 2.5 carats. The plot shows that the frequency  decreases as the weight increases.](quarto/diamond-sizes-notebook.png)
...
1. Re-create the document in [Figure 28.5](#fig-visual-editor) using the visual editor.
...
1. You can more easily navigate to specific chunks using the drop-down code navigator in the bottom-left of the script editor:
   ...
   ![Snippet of RStudio IDE showing only the drop-down code navigator  which shows three chunks. Chunk 1 is setup. Chunk 2 is cars and  it is in a section called Quarto. Chunk 3 is pressure and it is in  a section called Including plots.](screenshots/quarto-chunk-nav.png)
2. Graphics produced by the chunks will have useful names that make them easier to use elsewhere. More on that in [Section 28.6](#sec-figures).
3. You can set up networks of cached chunks to avoid re-performing expensive computations on every run. More on that in [Section 28.8](#sec-caching).
...
![Scatterplot of highway mileage vs. displacement of cars, where the points  are normally sized and the axis text and labels are in similar font size  to the surrounding text.](quarto_files/figure-html/unnamed-chunk-7-1.png)

![Scatterplot of highway mileage vs. displacement of cars, where the points  are smaller than in the previous plot and the axis text and labels are  smallter than the surrounding text.](quarto_files/figure-html/unnamed-chunk-8-1.png)

![Scatterplot of highway mileage vs. displacement of cars, where the points  are even smaller than in the previous plot and the axis text and labels are  even smallter than the surrounding text.](quarto_files/figure-html/unnamed-chunk-9-1.png)
...
If you prefer that data be displayed with additional formatting you can use the `knitr::kable()` function. The code below generates [Table 28.1](#tbl-kable).
...
While this introduction should be sufficient to get you started with Quarto, there is still a lot more to learn. Quarto is still relatively young, and is still growing rapidly. The best place to stay on top of innovations is the official Quarto website: [https://quarto.org](https://quarto.org/).
