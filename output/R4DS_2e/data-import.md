---
source_url: https://r4ds.hadley.nz/data-import.html
download_date: 2025-11-20
---

1. [Whole game](./whole-game.html)
2. [7  Data import](./data-import.html)
...
[Table 7.1](#tbl-students-table) shows a representation of the same data as a table.
...
When you run `read_csv()`, it prints out a message telling you the number of rows and columns of data, the delimiter that was used, and the column specifications (names of columns organized by the type of data the column contains). It also prints out some information about retrieving the full column specification and how to quiet this message. This message is an integral part of readr, and we’ll return to it in [Section 7.3](#sec-col-types).
...
An alternative approach is to use `janitor::clean_names()` to use some heuristics to turn them all into snake case at once[1](#fn1).
...
Note that the values in the `meal_plan` variable have stayed the same, but the type of variable denoted underneath the variable name has changed from character (`<chr>`) to factor (`<fct>`). You’ll learn more about factors in [Chapter 16](factors.html).

Before you analyze these data, you’ll probably want to fix the `age` column. Currently, `age` is a character variable because one of the observations is typed out as `five` instead of a numeric `5`. We discuss the details of fixing this issue in [Chapter 20](spreadsheets.html).
...
readr uses a heuristic to figure out the column types. For each column, it pulls the values of 1,000 rows[2](#fn2) spaced evenly from the first row to the last, ignoring missing values. It then works through the following questions:

* Does it contain only `F`, `T`, `FALSE`, or `TRUE` (ignoring case)? If so, it’s a logical.
* Does it contain only numbers (e.g., `1`, `-4.5`, `5e6`, `Inf`)? If so, it’s a number.
* Does it match the ISO8601 standard? If so, it’s a date or date-time. (We’ll return to date-times in more detail in [Section 17.2](datetimes.html#sec-creating-datetimes)).
...
#>   <int> <int> <chr>    <chr>  <chr>                           
#> 1     3     1 a double .      /tmp/RtmpK1pnGt/file1f2a7ec5df50
```
...
* `col_factor()`, `col_date()`, and `col_datetime()` create factors, dates, and date-times respectively; you’ll learn more about those when we get to those data types in [Chapter 16](factors.html) and [Chapter 17](datetimes.html).
* `col_number()` is a permissive numeric parser that will ignore non-numeric components, and is particularly useful for currencies. You’ll learn more about it in [Chapter 13](numbers.html).
...
If you have many files you want to read in, it can get cumbersome to write out their names as a list. Instead, you can use the base `list.files()` function to find the files for you by matching a pattern in the file names. You’ll learn more about these patterns in [Chapter 15](regexps.html).
...
1. `write_rds()` and `read_rds()` are uniform wrappers around the base functions `readRDS()` and `saveRDS()`. These store data in R’s custom binary format called RDS. This means that when you reload the object, you are loading the *exact same* R object that you stored.
   ...
   #> 6          6 Güvenç Attila    Ice cream          Lunch only              6
   ```
2. The arrow package allows you to read and write parquet files, a fast binary file format that can be shared across programming languages. We’ll return to arrow in more depth in [Chapter 22](arrow.html).
...
In this chapter, you’ve learned how to load CSV files with `read_csv()` and to do your own data entry with `tibble()` and `tribble()`. You’ve learned how csv files work, some of the problems you might encounter, and how to overcome them. We’ll come to data import a few times in this book: [Chapter 20](spreadsheets.html) from Excel and Google Sheets, [Chapter 21](databases.html) will show you how to load data from databases, [Chapter 22](arrow.html) from parquet files, [Chapter 23](rectangling.html) from JSON, and [Chapter 24](webscraping.html) from websites.
...
1. The [janitor](http://sfirke.github.io/janitor/) package is not part of the tidyverse, but it offers handy functions for data cleaning and works well within data pipelines that use `|>`.[↩︎](#fnref1)
2. You can override the default of 1000 with the `guess_max` argument.[↩︎](#fnref2)