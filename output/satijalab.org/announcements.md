...
* **Integration workflow:**  
  Seurat v5 introduces a [streamlined integration](/seurat/articles/integration_introduction) and [data transfer](/seurat/articles/integration_mapping) workflows that performs integration in low-dimensional space, and improves speed and memory efficiency. The results of integration are not identical between the two workflows, but users can still run the [v4 integration workflow](/seurat/articles/integration_introduction) in Seurat v5 if they wish.

  In previous versions of Seurat, the integration workflow required a list of multiple Seurat objects as input. In Seurat v5, all the data can be kept as a single object, but prior to integration, users can simply split the layers. See our [introduction to integration](/seurat/articles/integration_introduction) vignette for more information.
...
* **Pseudobulk analysis:**  
  ...
  Check out our [differential expression vignette](/seurat/articles/de_vignette) as well as our [pancreatic/healthy PBMC comparison](/seurat/articles/parsebio_sketch_integration), for examples of how to use `AggregateExpression` to perform robust differential expression of scRNA-seq data from multiple different conditions.