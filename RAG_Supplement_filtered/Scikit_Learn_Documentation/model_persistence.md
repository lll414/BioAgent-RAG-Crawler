...
Source URL: https://scikit-learn.org/stable/model_persistence.html
...
Summary of model persistence methods[#](#id2 "Link to this table")
...
| --- | --- | --- |
| [ONNX](#onnx-persistence) | * Serve models without a Python environment * Serving and training environments independent of one another * Most secure option | * Not all scikit-learn models are supported * Custom estimators require more work to support * Original Python object is lost and cannot be reconstructed |
| [skops.io](#skops-persistence) | * More secure than `pickle` based formats * Contents can be partly validated without loading | * Not as fast as `pickle` based formats * Supports less types than `pickle` based formats * Requires the same environment as the training environment |
| [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)") | * Native to Python * Can serialize most Python objects * Efficient memory usage with `protocol=5` | * Loading can execute arbitrary code * Requires the same environment as the training environment |
| [`joblib`](https://joblib.readthedocs.io/en/latest/index.html#module-joblib "(in joblib v1.6.dev0)") | * Efficient memory usage * Supports memory mapping * Easy shortcuts for compression and decompression | * Pickle based format * Loading can execute arbitrary code * Requires the same environment as the training environment |
| [cloudpickle](https://github.com/cloudpipe/cloudpickle) | * Can serialize non-packaged, custom Python code * Comparable loading efficiency as [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)") with `protocol=5` | * Pickle based format * Loading can execute arbitrary code * No forward compatibility guarantees * Requires the same environment as the training environment |
...
object itself is required, then [ONNX](#onnx-persistence) might be the
...
If you have security concerns, then you should consider using [skops.io](#skops-persistence) which gives you back the Python object, but unlike
...
investigation of the persisted file, which [`skops.io`](https://skops.readthedocs.io/en/stable/modules/classes.html#module-skops.io "(in skops)") allows you to do.
...
If yes, then you can consider using [joblib](#pickle-persistence). If this
is not a major concern for you, then you can use the built-in [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)")
module.

4. Did you try [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)") or [`joblib`](https://joblib.readthedocs.io/en/latest/index.html#module-joblib "(in joblib v1.6.dev0)") and found that the model cannot
   be persisted? It can happen for instance when you have user defined
   functions in your model.

If yes, then you can use [cloudpickle](https://github.com/cloudpipe/cloudpickle) which can serialize certain objects
which cannot be serialized by [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)") or [`joblib`](https://joblib.readthedocs.io/en/latest/index.html#module-joblib "(in joblib v1.6.dev0)").

## 10.1. Workflow Overview[#](#workflow-overview "Link to this heading")
...
### 10.1.1. Train and Persist the Model[#](#train-and-persist-the-model "Link to this heading")

Creating an appropriate model depends on your use-case. As an example, here we
train a [`sklearn.ensemble.HistGradientBoostingClassifier`](modules/generated/sklearn.ensemble.HistGradientBoostingClassifier.html#sklearn.ensemble.HistGradientBoostingClassifier "sklearn.ensemble.HistGradientBoostingClassifier") on the iris
dataset:
...
* [ONNX](#onnx-persistence): You need an `ONNX` runtime and an environment
...
## 10.2. ONNX[#](#onnx "Link to this heading")

`ONNX`, or [Open Neural Network Exchange](https://onnx.ai/) format is best
...
architectures. More details are available from the [ONNX tutorial](https://onnx.ai/get-started.html). To convert scikit-learn model to `ONNX`
[sklearn-onnx](http://onnx.ai/sklearn-onnx/) has been developed. However,
...
Using ONNX[#](#using-onnx "Link to this dropdown")

To convert the model to `ONNX` format, you need to give the converter some
information about the input as well, about which you can read more [here](http://onnx.ai/sklearn-onnx/index.html):
...
## 10.3. `skops.io`[#](#skops-io "Link to this heading")

[`skops.io`](https://skops.readthedocs.io/en/stable/modules/classes.html#module-skops.io "(in skops)") avoids using [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)") and only loads files which have types
and references to functions which are trusted either by default or by the user.
Therefore it provides a more secure format than [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)"), [`joblib`](https://joblib.readthedocs.io/en/latest/index.html#module-joblib "(in joblib v1.6.dev0)"),
and [cloudpickle](https://github.com/cloudpipe/cloudpickle).

Using skops[#](#using-skops "Link to this dropdown")

The API is very similar to [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)"), and you can persist your models as
explained in the [documentation](https://skops.readthedocs.io/en/stable/persistence.html) using
[`skops.io.dump`](https://skops.readthedocs.io/en/stable/modules/classes.html#skops.io.dump "(in skops)") and [`skops.io.dumps`](https://skops.readthedocs.io/en/stable/modules/classes.html#skops.io.dumps "(in skops)"):
...
And you can load them back using [`skops.io.load`](https://skops.readthedocs.io/en/stable/modules/classes.html#skops.io.load "(in skops)") and
[`skops.io.loads`](https://skops.readthedocs.io/en/stable/modules/classes.html#skops.io.loads "(in skops)"). However, you need to specify the types which are
trusted by you. You can get existing unknown types in a dumped object / file
using [`skops.io.get_untrusted_types`](https://skops.readthedocs.io/en/stable/modules/classes.html#skops.io.get_untrusted_types "(in skops)"), and after checking its contents,
...
issue tracker](https://github.com/skops-dev/skops/issues).

## 10.4. `pickle`, `joblib`, and `cloudpickle`[#](#pickle-joblib-and-cloudpickle "Link to this heading")

These three modules / packages, use the `pickle` protocol under the hood, but
come with slight variations:

* [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)") is a module from the Python Standard Library. It can serialize
  and deserialize any Python object, including custom Python classes and
  objects.
* [`joblib`](https://joblib.readthedocs.io/en/latest/index.html#module-joblib "(in joblib v1.6.dev0)") is more efficient than `pickle` when working with large machine
  learning models or large numpy arrays.
* [cloudpickle](https://github.com/cloudpipe/cloudpickle) can serialize certain objects which cannot be serialized by
  [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)") or [`joblib`](https://joblib.readthedocs.io/en/latest/index.html#module-joblib "(in joblib v1.6.dev0)"), such as user defined functions and lambda
  functions. This can happen for instance, when using a
  [`FunctionTransformer`](modules/generated/sklearn.preprocessing.FunctionTransformer.html#sklearn.preprocessing.FunctionTransformer "sklearn.preprocessing.FunctionTransformer") and using a custom
  function to transform the data.

Using `pickle`, `joblib`, or `cloudpickle`[#](#using-pickle,-joblib,-or-cloudpickle "Link to this dropdown")
...
## 10.5. Security & Maintainability Limitations[#](#security-maintainability-limitations "Link to this heading")

[`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)") (and [`joblib`](https://joblib.readthedocs.io/en/latest/index.html#module-joblib "(in joblib v1.6.dev0)") and `cloudpickle` by extension), has
...
different version of scikit-learn. While using [`skops.io`](https://skops.readthedocs.io/en/stable/modules/classes.html#module-skops.io "(in skops)"), [`joblib`](https://joblib.readthedocs.io/en/latest/index.html#module-joblib "(in joblib v1.6.dev0)"),
[`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)"), or [cloudpickle](https://github.com/cloudpipe/cloudpickle), models saved using one version of
...
* [Adrin Jalali: Let’s exploit pickle, and skops to the rescue! | PyData
  Amsterdam 2023](https://www.youtube.com/watch?v=9w_H5OSTO9A).
...
### 10.5.1. Replicating the training environment in production[#](#replicating-the-training-environment-in-production "Link to this heading")
...
InconsistentVersionWarning[#](#inconsistentversionwarning "Link to this dropdown")
...
[`InconsistentVersionWarning`](modules/generated/sklearn.exceptions.InconsistentVersionWarning.html#sklearn.exceptions.InconsistentVersionWarning "sklearn.exceptions.InconsistentVersionWarning") is raised. This warning
...
### 10.5.2. Serving the model artifact[#](#serving-the-model-artifact "Link to this heading")
...
## 10.6. Summarizing the key points[#](#summarizing-the-key-points "Link to this heading")
...
* [`skops.io`](https://skops.readthedocs.io/en/stable/modules/classes.html#module-skops.io "(in skops)"): Trained scikit-learn models can be easily shared and put
  into production using [`skops.io`](https://skops.readthedocs.io/en/stable/modules/classes.html#module-skops.io "(in skops)"). It is more secure compared to
  alternate approaches based on [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)") because it does not load
...
* [`joblib`](https://joblib.readthedocs.io/en/latest/index.html#module-joblib "(in joblib v1.6.dev0)"): Efficient memory mapping techniques make it faster when using
...
* [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)"): It is native to Python and most Python objects can be
  serialized and deserialized using [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)"), including custom Python
  classes and functions as long as they are defined in a package that can be
  imported in the target environment. While [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)") can be used to easily
  save and load scikit-learn models, it may trigger the execution of malicious
  code while loading a model from an untrusted source. [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)") can also
  be very efficient memorywise if the model was persisted with `protocol=5` but
  it does not support memory mapping.
* [cloudpickle](https://github.com/cloudpipe/cloudpickle): It has comparable loading efficiency as [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.14)") and
  [`joblib`](https://joblib.readthedocs.io/en/latest/index.html#module-joblib "(in joblib v1.6.dev0)") (without memory mapping), but offers additional flexibility to
  ...
  [`sklearn.preprocessing.FunctionTransformer`](modules/generated/sklearn.preprocessing.FunctionTransformer.html#sklearn.preprocessing.FunctionTransformer "sklearn.preprocessing.FunctionTransformer") that wraps a function
  defined in the training script itself or more generally outside of any
  importable Python package. Note that [cloudpickle](https://github.com/cloudpipe/cloudpickle) offers no forward
  compatibility guarantees and you might need the same version of
  [cloudpickle](https://github.com/cloudpipe/cloudpickle) to load the persisted model along with the same version of all
...
9.3. Parallelism, resource management, and configuration](computing/parallelism.html "previous page")
[next

11. Common pitfalls and recommended practices](common_pitfalls.html "next page")

On this page

* [10.1. Workflow Overview](#workflow-overview)
  + [10.1.1. Train and Persist the Model](#train-and-persist-the-model)
* [10.2. ONNX](#onnx)
* [10.3. `skops.io`](#skops-io)
* [10.4. `pickle`, `joblib`, and `cloudpickle`](#pickle-joblib-and-cloudpickle)
* [10.5. Security & Maintainability Limitations](#security-maintainability-limitations)
  + [10.5.1. Replicating the training environment in production](#replicating-the-training-environment-in-production)
  + [10.5.2. Serving the model artifact](#serving-the-model-artifact)
* [10.6. Summarizing the key points](#summarizing-the-key-points)

### This Page

* [Show Source](_sources/model_persistence.rst.txt)