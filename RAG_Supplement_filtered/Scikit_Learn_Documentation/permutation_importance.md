...
Source URL: https://scikit-learn.org/stable/modules/permutation_importance.html
...
contribution of each feature to a [fitted](../glossary.html#term-fitted) model’s statistical performance
on a given tabular dataset. This technique is particularly useful for non-linear
or opaque [estimators](../glossary.html#term-estimators), and involves randomly shuffling the values of a
single feature and observing the resulting degradation of the model’s score
[[1]](#id2). By breaking the relationship between the feature and the target, we
...
![../_images/permuted_predictive_feature.png](../_images/permuted_predictive_feature.png)
![../_images/permuted_non_predictive_feature.png](../_images/permuted_non_predictive_feature.png)
...
[`RandomForestClassifier`](generated/sklearn.ensemble.RandomForestClassifier.html#sklearn.ensemble.RandomForestClassifier "sklearn.ensemble.RandomForestClassifier") trained on an augmented
...
of [estimators](../glossary.html#term-estimators) for a given dataset. The `n_repeats` parameter sets the
...
[`permutation_importance`](generated/sklearn.inspection.permutation_importance.html#sklearn.inspection.permutation_importance "sklearn.inspection.permutation_importance") function to probe which features are most
predictive:
...
[`permutation_importance`](generated/sklearn.inspection.permutation_importance.html#sklearn.inspection.permutation_importance "sklearn.inspection.permutation_importance") several times with a different scorer, as it
reuses model predictions.

Example of permutation feature importance using multiple scorers[#](#example-of-permutation-feature-importance-using-multiple-scorers "Link to this dropdown")

In the example below we use a list of metrics, but more input formats are
possible, as documented in [Using multiple metric evaluation](model_evaluation.html#multimetric-scoring).
...
## 5.2.1. Outline of the permutation importance algorithm[#](#outline-of-the-permutation-importance-algorithm "Link to this heading")
...
* For each feature \(j\) (column of \(D\)):
  ...
  + For each repetition \(k\) in \({1, ..., K}\):
    ...
    - Compute the score \(s\_{k,j}\) of model \(m\) on corrupted data
      \(\tilde{D}\_{k,j}\).
  + Compute importance \(i\_j\) for feature \(f\_j\) defined as:

    \[i\_j = s - \frac{1}{K} \sum\_{k=1}^{K} s\_{k,j}\]

## 5.2.2. Relation to impurity-based importance in trees[#](#relation-to-impurity-based-importance-in-trees "Link to this heading")

Tree-based models provide an alternative measure of [feature importances
based on the mean decrease in impurity](ensemble.html#random-forest-feature-importance)
...
## 5.2.3. Misleading values on strongly correlated features[#](#misleading-values-on-strongly-correlated-features "Link to this heading")
...
The figure below shows the permutation feature importance of a
[`RandomForestClassifier`](generated/sklearn.ensemble.RandomForestClassifier.html#sklearn.ensemble.RandomForestClassifier "sklearn.ensemble.RandomForestClassifier") trained using the
[Breast cancer Wisconsin (diagnostic) dataset](../datasets/toy_dataset.html#breast-cancer-dataset), which contains strongly correlated features. A
...
L. Breiman, [“Random Forests”](https://doi.org/10.1023/A:1010933404324),
Machine Learning, 45(1), 5-32, 2001.

[previous

5.1. Partial Dependence and Individual Conditional Expectation plots](partial_dependence.html "previous page")
[next

6. Visualizations](../visualizations.html "next page")

On this page

* [5.2.1. Outline of the permutation importance algorithm](#outline-of-the-permutation-importance-algorithm)
* [5.2.2. Relation to impurity-based importance in trees](#relation-to-impurity-based-importance-in-trees)
* [5.2.3. Misleading values on strongly correlated features](#misleading-values-on-strongly-correlated-features)

### This Page

* [Show Source](../_sources/modules/permutation_importance.rst.txt)