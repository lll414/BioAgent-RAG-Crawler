...
Source URL: https://scikit-learn.org/stable/modules/biclustering.html
...
An example of biclusters formed by partitioning rows and columns.[#](#id5 "Link to this image")
...
An example of checkerboard biclusters.[#](#id6 "Link to this image")
...
## 2.4.1. Spectral Co-Clustering[#](#spectral-co-clustering "Link to this heading")

The [`SpectralCoclustering`](generated/sklearn.cluster.SpectralCoclustering.html#sklearn.cluster.SpectralCoclustering "sklearn.cluster.SpectralCoclustering") algorithm finds biclusters with
...
### 2.4.1.1. Mathematical formulation[#](#mathematical-formulation "Link to this heading")
...
\(\sum\_{j} A\_{ij}\) and \(C\) is the diagonal matrix with
entry \(j\) equal to \(\sum\_{i} A\_{ij}\).

The singular value decomposition, \(A\_n = U \Sigma V^\top\),
...
The \(\ell = \lceil \log\_2 k \rceil\) singular vectors, starting
...
where the columns of \(U\) are \(u\_2, \dots, u\_{\ell +
1}\), and similarly for \(V\).

Then the rows of \(Z\) are clustered using [k-means](clustering.html#k-means). The first `n_rows` labels provide the row partitioning,
and the remaining `n_columns` labels provide the column partitioning.

Examples

* [A demo of the Spectral Co-Clustering algorithm](../auto_examples/bicluster/plot_spectral_coclustering.html#sphx-glr-auto-examples-bicluster-plot-spectral-coclustering-py): A simple example
  showing how to generate a data matrix with biclusters and apply
  this method to it.
* [Biclustering documents with the Spectral Co-clustering algorithm](../auto_examples/bicluster/plot_bicluster_newsgroups.html#sphx-glr-auto-examples-bicluster-plot-bicluster-newsgroups-py): An example of finding
...
## 2.4.2. Spectral Biclustering[#](#spectral-biclustering "Link to this heading")

The [`SpectralBiclustering`](generated/sklearn.cluster.SpectralBiclustering.html#sklearn.cluster.SpectralBiclustering "sklearn.cluster.SpectralBiclustering") algorithm assumes that the input
...
### 2.4.2.1. Mathematical formulation[#](#id3 "Link to this heading")
...
3. **Log normalization**: the log of the data matrix is computed: \(L =
   \log A\). Then the column mean \(\overline{L\_{i \cdot}}\), row mean
   \(\overline{L\_{\cdot j}}\), and overall mean \(\overline{L\_{\cdot
   \cdot}}\) of \(L\) are computed. The final matrix is computed
   according to the formula

\[K\_{ij} = L\_{ij} - \overline{L\_{i \cdot}} - \overline{L\_{\cdot
...
were used, the first singular vectors, \(u\_1\) and \(v\_1\).
are discarded. From now on, the “first” singular vectors refers to
\(u\_2 \dots u\_{p+1}\) and \(v\_2 \dots v\_{p+1}\) except in the
...
* [A demo of the Spectral Biclustering algorithm](../auto_examples/bicluster/plot_spectral_biclustering.html#sphx-glr-auto-examples-bicluster-plot-spectral-biclustering-py): a simple example
  showing how to generate a checkerboard matrix and bicluster it.

References

* Kluger, Yuval, et. al., 2003. [Spectral biclustering of microarray
  data: coclustering genes and conditions](https://doi.org/10.1101/gr.648603)

## 2.4.3. Biclustering evaluation[#](#biclustering-evaluation "Link to this heading")
...
For now, only [`consensus_score`](generated/sklearn.metrics.consensus_score.html#sklearn.metrics.consensus_score "sklearn.metrics.consensus_score") (Hochreiter et. al., 2010) is
available:
...
2. Assign biclusters from one set to another in a one-to-one fashion
   ...
   using [`scipy.optimize.linear_sum_assignment`](https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.linear_sum_assignment.html#scipy.optimize.linear_sum_assignment "(in SciPy v1.16.2)"), which uses a
...
2.3. Clustering](clustering.html "previous page")
[next

2.5. Decomposing signals in components (matrix factorization problems)](decomposition.html "next page")

On this page

* [2.4.1. Spectral Co-Clustering](#spectral-co-clustering)
  + [2.4.1.1. Mathematical formulation](#mathematical-formulation)
* [2.4.2. Spectral Biclustering](#spectral-biclustering)
  + [2.4.2.1. Mathematical formulation](#id3)
* [2.4.3. Biclustering evaluation](#biclustering-evaluation)

### This Page

* [Show Source](../_sources/modules/biclustering.rst.txt)