---
source_url: https://r4ds.hadley.nz/rectangling.html
download_date: 2025-11-20
---

1. [Import](./import.html)
2. [23  Hierarchical data](./rectangling.html)
...
As lists get even larger and more complex, `str()` eventually starts to fail, and you’ll need to switch to `View()`[1](#fn1). [Figure 23.1](#fig-view-collapsed) shows the result of calling `View(x5)`. The viewer starts by showing just the top level of the list, but you can interactively expand any of the components to see more, as in [Figure 23.2](#fig-view-expand-1). RStudio will also show you the code you need to access that element, as in [Figure 23.3](#fig-view-expand-2). We’ll come back to how this code works in [Section 27.3](base-R.html#sec-subset-one).

![A screenshot of RStudio showing the list-viewer. It shows the two children of x5: the first child is a double vector and the second child is a list. A rightward facing triable indicates that the second child itself has children but you can't see them. ](screenshots/View-1.png)
...
![Another screenshot of the list-viewer having expand the second child of x5. It also has two children, a double vector and another list. ](screenshots/View-2.png)
...
![Another screenshot, having expanded the grandchild of x5 to see its two children, again a double vector and a list. ](screenshots/View-3.png)
...
Computing with list-columns is harder, but that’s because computing with lists is harder in general; we’ll come back to that in [Chapter 26](iteration.html). In this chapter, we’ll focus on unnesting list-columns out into regular variables so you can use your existing tools on them.
...
Dealing with inconsistent types is challenging and the details depend on the precise nature of the problem and your goals, but you’ll most likely need tools from [Chapter 26](iteration.html).
...
1. This is an RStudio feature.[↩︎](#fnref1)
