---
source_url: https://r4ds.hadley.nz/databases.html
download_date: 2025-11-20
---

1. [Import](./import.html)
2. [21  Databases](./databases.html)
...
Concretely, you create a database connection using `DBI::dbConnect()`. The first argument selects the DBMS[2](#fn2), then the second and subsequent arguments describe how to connect to it (i.e. where it lives and the credentials that you need to access it). The following code shows a couple of typical examples:
...
We’ll come back to the translation of individual components (like `/`) in [Section 21.6](#sec-sql-expressions).
...
`group_by()` is translated to the `GROUP BY`[6](#fn6) clause and `summarize()` is translated to the `SELECT` clause:
...
We’ll come back to what’s happening with the translation of `n()` and `mean()` in [Section 21.6](#sec-sql-expressions).
...
1. SQL is either pronounced “s”-“q”-“l” or “sequel”.[︎](#fnref1)
2. Typically, this is the only function you’ll use from the client package, so we recommend using `::` to pull out that one function, rather than loading the complete package with `library()`.[︎](#fnref2)
3. At least, all the tables that you have permission to see.[︎](#fnref3)
4. Confusingly, depending on the context, `SELECT` is either a statement or a clause. To avoid this confusion, we’ll generally use `SELECT` query instead of `SELECT` statement.[︎](#fnref4)
5. Ok, technically, only the `SELECT` is required, since you can write queries like `SELECT 1+1` to perform basic calculations. But if you want to work with data (as you always do!) you’ll also need a `FROM` clause.[︎](#fnref5)
6. This is no coincidence: the dplyr function name was inspired by the SQL clause.[︎](#fnref6)
