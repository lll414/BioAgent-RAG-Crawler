---
source_url: https://r4ds.hadley.nz/missing-values.html
download_date: 2025-11-20
---

1. [Transform](./transform.html)
2. [18  Missing values](./missing-values.html)

# 18  Missing values

## 18.1 Introduction

You’ve already learned the basics of missing values earlier in the book. You first saw them in [Chapter 1](data-visualize.html) where they resulted in a warning when making a plot as well as in [Section 3.5.2](data-transform.html#sec-summarize) where they interfered with computing summary statistics, and you learned about their infectious nature and how to check for their presence in [Section 12.2.2](logicals.html#sec-na-comparison). Now we’ll come back to them in more depth, so you can learn more of the details.

We’ll start by discussing some general tools for working with missing values recorded as `NA`s. We’ll then explore the idea of implicitly missing values, values that are simply absent from your data, and show some tools you can use to make them explicit. We’ll finish off with a related discussion of empty groups, caused by factor levels that don’t appear in the data.
...
By default, making data longer preserves explicit missing values, but if they are structurally missing values that only exist because the data is not tidy, you can drop them (make them implicit) by setting `values_drop_na = TRUE`. See the examples in [Section 5.2](data-tidy.html#sec-tidy-data) for more details.
...
This brings us to another important way of revealing implicitly missing observations: joins. You’ll learn more about joins in [Chapter 19](joins.html), but we wanted to quickly mention them to you here since you can often only know that values are missing from one dataset when you compare it to another.
...
This dataset only contains non-smokers, but we know that smokers exist; the group of smokers is empty. We can request `count()` to keep all the groups, even those not seen in the data by using `.drop = FALSE`:
...
![A bar chart with a single value on the x-axis, "no".](missing-values_files/figure-html/unnamed-chunk-16-1.png)

![The same bar chart as the last plot, but now with two values on the x-axis, "yes" and "no". There is no bar for the "yes" category.](missing-values_files/figure-html/unnamed-chunk-16-2.png)
...
1. In other words, `min(c(x, y))` is always equal to `min(min(x), min(y))`.[︎](#fnref1)
